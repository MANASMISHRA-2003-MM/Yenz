...
export default function AdminDashboard() {
  ...
  const fetchAdminData = async () => {
    try {
      setLoading(true);
      const [resMetrics, resAnalytics, resUsers, resOrders] = await Promise.all([
      ...
      if (resUsers.data.success) {
        ...
        setDrivers(resUsers.data.users.filter(u => u.role === 'delivery_partner'));
      }
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 pb-28">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
          ...
          <div className="space-y-8">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-white p-5 rounded-3xl border border-slate-200/90 shadow-soft">
                <p className="text-[10px] text-slate-400 font-extrabold uppercase tracking-wider">Total Platform Revenue</p>
                <p className="text-2xl font-extrabold text-emerald-600 mt-1">₹{metrics?.revenue || 0}</p>
              </div>
              <div className="bg-white p-5 rounded-3xl border border-slate-200/90 shadow-soft">
                <p className="text-[10px] text-slate-400 font-extrabold uppercase tracking-wider">Total Orders Placed</p>
                <p className="text-2xl font-extrabold text-slate-900 mt-1">{metrics?.totalOrders || 0}</p>
              </div>
              <div className="bg-white p-5 rounded-3xl border border-slate-200/90 shadow-soft">
                <p className="text-[10px] text-slate-400 font-extrabold uppercase tracking-wider">Active Vendors</p>
                <p className="text-2xl font-extrabold text-amber-600 mt-1">{metrics?.activeVendors || 0}</p>
              </div>
              <div className="bg-white p-5 rounded-3xl border border-slate-200/90 shadow-soft">
                <p className="text-[10px] text-slate-400 font-extrabold uppercase tracking-wider">Active Delivery Partners</p>
                <p className="text-2xl font-extrabold text-cyan-600 mt-1">{metrics?.totalDeliveryPartners || 0}</p>
              </div>
            </div>
        ...
        {/* ORDERS MANAGEMENT & MANUAL ASSIGNMENT TAB */}
        {activeTab === 'orders' && (
          <div className="bg-white rounded-3xl border border-slate-200/90 shadow-soft overflow-hidden">
            <div className="p-4 border-b border-slate-100 font-extrabold text-sm text-slate-900">
                ...
                <tbody className="divide-y divide-slate-100 font-medium">
                  {orders.map(o => (
                    <tr key={o._id} className="hover:bg-slate-50">
                      ...
                      <td className="p-3">{o.customerId?.name}</td>
                      <td className="p-3">{o.restaurantId?.name}</td>
                      <td className="p-3 font-extrabold text-brand-600">₹{o.totalAmount}</td>
                      <td className="p-3"><OrderStatusBadge status={o.status} /></td>
                      <td className="p-3 text-slate-700 font-bold">{o.deliveryPartnerId?.name || 'Unassigned'}</td>
                      <td className="p-3">
                        <select
                          value={o.deliveryPartnerId?._id || ''}
